...
contract RWAETFVaultTest is Test {
    ...
    function testUpdateAsset() public {
        ...
        // Verify update
        (, uint256 targetWeight, uint256 minWeight, uint256 maxWeight, ) = vault
            .assets(0);
    ...
    uint256[] memory amounts = new uint256[](1);
    tokens[0] = address(rwaToken1);
    amounts[0] = 100 * 10 ** 18; // $1000 value (100 tokens at $10 each)
    ...
    // Calculate required deposit for 10% of vault
    uint256 vaultValue = vault.getTotalVaultValue(); // $1000
    uint256 valuePerSquare = vaultValue / 100; // $10 per square
    uint256 requiredValue = valuePerSquare * numberOfSquares; // $100 total

    // Calculate tokens needed (with buffer for rounding)
    uint256 tokenPrice = priceOracle.prices(address(rwaToken1));
    uint256 tokensNeeded = (requiredValue * 1e18) / tokenPrice;
    // Add 1 to avoid rounding issues
    tokensNeeded += 1;

    vm.startPrank(user2);
    address[] memory user2Tokens = new address[](1);
    uint256[] memory user2Amounts = new uint256[](1);
    user2Tokens[0] = address(rwaToken1);
    user2Amounts[0] = tokensNeeded;

    rwaToken1.approve(address(vault), user2Amounts[0]);

    // Calculate expected ETF amount
    uint256 expectedETF = (etfToken.totalSupply() * numberOfSquares) / 100;

    vm.expectEmit(true, false, false, true);
    emit FractionalOwnershipPurchased(user2, numberOfSquares, expectedETF);

    vault.buyVaultFractions(numberOfSquares, user2Tokens, user2Amounts);
    vm.stopPrank();
    ...
    function testGetCurrentWeights() public {
        ...
        (address[] memory tokens, uint256[] memory weights) = vault
            .getCurrentWeights();
